...
def remove_punct(text):
    ...
    'goSouTh'
    """
    punct = '''!()-[]{};:'"\,<>./?@#$%^&*_~'''  # Dictates the punctuation
    texr = ""  # makes text nothing
    c_punct = ""  # Makes c_punct  nothing
    for char in text:  # it loops char in texts
        if char not in punct:
            c_punct = c_punct + char  # adds charcters to c_punct
    return (c_punct)  # returns c_punct
...
def remove_spaces(text):
    ...
    ''
    """
    return text.strip()#strips the white spaces from a string and returns the string
...
def normalise_input(user_input):
    ...
    """
    inpu= ""
    inpu = user_input.lower()#Makes inpu lowercase
    inpu = remove_punct(inpu)#Removes punctuation from inpu
    inpu = remove_spaces(inpu)#removes spaces from inpu
    print("\'" + inpu + "\'") 
...
def display_room(room):
    ...
    print()#Empty line
    print(room["name"].upper())#Name of area 
    print()
    print(room["description"])#gives discription of area 
    print()
...
def exit_leads_to(exits, direction):
    ...
    'Reception'
    """
    variable_exit_leads = rooms[exits[direction]]["name"]
        
    return variable_exit_leads # returns what the room is called
    #pass
...
def print_menu_line(direction, leads_to):
    ...
    """
    print ("Go " + direction.upper() + " to " + leads_to + ".")#prints the direction (east, south, west) and area it goes (tutor room 
...
def is_valid_exit(exits, user_input):
    ...
    """
    for x in exits:
        if user_input == x:  # if uswe input is the same as a which is exits
            return True
            break # true is printed
        # stop the for loop
    else:
...
def menu(exits):
    ...
    while True:
        print_menu(exits)#Call menu exits function 

        entered = input() #Enter a useable input to decide where to go next
        normalise_input(entered)#Call function normalise~_input

        if is_valid_exit(exits, entered) == True:#if enetered number is vaild it will return the entered number
            return entered 
            break       
...
def move(exits, direction):
    ...
    False
    """
    return rooms[exits[direction]]# It will return the function room
